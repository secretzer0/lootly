...
class TestAccountApi(BaseApiTest):
    ...
    async def test_get_seller_standards_error_handling(self, mock_context, mock_credentials):
        ...
        if self.is_integration_mode:
            ...
            data = json.loads(result)
            assert data["status"] == "error"
            # Will be auth error without user consent, or validation error if we had consent
            assert data["error_code"] in ["EXTERNAL_API_ERROR", "VALIDATION_ERROR", "AUTHENTICATION_ERROR"]
        else:
