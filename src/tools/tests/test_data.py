...
class TestDataFulfillmentPolicy:
    ...
    def create_complex_policy(cls, name: str = "Premium Shipping") -> FulfillmentPolicyInput:
        ...
        domestic_option = ShippingOption(
            ...
            shippingServices=[domestic_standard, domestic_expedited],
            packageHandlingCost=Amount(currency=CurrencyCodeEnum.USD, value="1.99")
        )
        
        international_option = ShippingOption(
            costType=ShippingCostTypeEnum.FLAT_RATE,
            optionType=ShippingOptionTypeEnum.INTERNATIONAL,
            shippingServices=[international_standard]
        )
    ...
    def policy_to_api_response(cls, policy: FulfillmentPolicyInput, policy_id: str = "6197932000") -> dict:
        ...
        response = {
            ...
            "name": policy.name,
            "marketplaceId": policy.marketplaceId.value,
            "categoryTypes": [
                {"name": cat.name.value, **({"default": cat.default} if cat.default is not None else {})}
                for cat in policy.categoryTypes
            ]
        }
        ...
        if policy.handlingTime:
            response["handlingTime"] = {
                "value": policy.handlingTime.value,
                "unit": policy.handlingTime.unit.value
            }
            
        if policy.shippingOptions:
            response["shippingOptions"] = []
            for opt in policy.shippingOptions:
                opt_data = {
                    "costType": opt.costType.value,
                    "optionType": opt.optionType.value
                }
                
                if opt.packageHandlingCost:
                    opt_data["packageHandlingCost"] = {
                        "currency": opt.packageHandlingCost.currency.value,
                        "value": opt.packageHandlingCost.value
                    }
                
                if opt.shippingServices:
                    opt_data["shippingServices"] = []
                    for svc in opt.shippingServices:
                        svc_data = {"shippingServiceCode": svc.shippingServiceCode}
                        
                        if svc.shippingCarrierCode:
                            svc_data["shippingCarrierCode"] = svc.shippingCarrierCode
                        if svc.shippingCost:
                            svc_data["shippingCost"] = {
                                "currency": svc.shippingCost.currency.value,
                                "value": svc.shippingCost.value
                            }
                        if svc.additionalShippingCost:
                            svc_data["additionalShippingCost"] = {
                                "currency": svc.additionalShippingCost.currency.value,
                                "value": svc.additionalShippingCost.value
                            }
                        if svc.freeShipping is not None:
                            svc_data["freeShipping"] = svc.freeShipping
                        if svc.sortOrder is not None:
                            svc_data["sortOrder"] = svc.sortOrder
                            
                        opt_data["shippingServices"].append(svc_data)
                
                response["shippingOptions"].append(opt_data)
        
        if policy.localPickup is not None:
            response["localPickup"] = policy.localPickup
        if policy.pickupDropOff is not None:
            response["pickupDropOff"] = policy.pickupDropOff
        if policy.freightShipping is not None:
            response["freightShipping"] = policy.freightShipping
        if policy.globalShipping is not None:
            response["globalShipping"] = policy.globalShipping
