...
class TestCompleteSellerWorkflow:
    ...
    async def test_complete_seller_workflow_static_fallback(self, mock_context):
        ...
        # Step 4: Get seller standards  
        standards_result = await get_seller_standards.fn(
            ctx=mock_context,
            program="PROGRAM_US",
            cycle="CURRENT"
        )
        
        standards_data = json.loads(standards_result)
        assert standards_data["status"] == "success"
        assert standards_data["data"]["data_source"] == "static_fallback"
        assert "seller_standards" in standards_data["data"]
    ...
    async def test_workflow_with_policies(self, mock_context):
        ...
        # Get seller standards for different programs
        us_standards = await get_seller_standards.fn(
            ctx=mock_context,
            program="PROGRAM_US",
            cycle="CURRENT"
        )
        
        global_standards = await get_seller_standards.fn(
            ctx=mock_context,
            program="PROGRAM_GLOBAL",
            cycle="CURRENT"
        )
        
        projected_standards = await get_seller_standards.fn(
            ctx=mock_context,
            program="PROGRAM_US",
            cycle="PROJECTED"
        )
        
        # Verify all standard types work
        for standards_result in [us_standards, global_standards, projected_standards]:
            standards_data = json.loads(standards_result)
            assert standards_data["status"] == "success"
            assert standards_data["data"]["data_source"] == "static_fallback"
            assert "seller_standards" in standards_data["data"]
        ...
        # Create offer (static fallback will create basic offer)
        offer_result = await create_offer.fn(
            ctx=mock_context,
            sku="TEST-POLICIES-001",
            category_id="166",
...
class TestApiInteroperability:
    ...
    async def test_progressive_enhancement(self, mock_context):
        ...
        api_calls = [
            ...
            ("seller_standards", get_seller_standards.fn(ctx=mock_context, program="PROGRAM_US")),
